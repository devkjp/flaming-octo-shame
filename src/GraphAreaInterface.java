...
public abstract class GraphAreaInterface extends JPanel {
	...
	public static enum color {
		BLANK, BLACK, RED, WHITE, GRAY, TURN
	};
	...
	public void setPixelWidth(int pxWidth) {
		this.pixelWidth = pxWidth;
		pixel = new color[this.graphAreaWidth / pixelWidth][this.graphAreaHeight / pixelWidth];
	...
	public void setTransformationBasePoint(int canvasX, int canvasY) {
		...
		for (int i = 0; i < 1; i++) {
			for (int j = 0; j < 1; j++) {
				if (0 <= canvasX + i && canvasX + i < pixel[0].length
						&& 0 <= canvasY + j && canvasY + j < pixel.length)
